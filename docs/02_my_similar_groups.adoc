...
image::{img}/slides.jpg[]
pass:a[<a play-topic='{guides}/02_find_yourself_answers.html'>Answers: Explore the graph</a>]
...
----
MATCH (member:Member) WHERE member.name CONTAINS '<span  value-key="name">A Name</span>'
MATCH (group:Group)-[:HAS_TOPIC]->(topic)<-[:HAS_TOPIC]-(otherGroup:Group)
WHERE group.name CONTAINS {neo4j-group-name}
...
WHERE member.name CONTAINS '<span  value-key="name">A Name</span>'
  AND NOT (member)-[:MEMBER_OF]->(otherGroup)
RETURN otherGroup.name,
