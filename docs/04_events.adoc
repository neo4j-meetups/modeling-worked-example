...
We'll give a score of `5` if it's an event in one of our groups, add that to the number of common topics and then order by the total.
...
RETURN futureEvent.name, futureEvent.time, group.name, commonTopics, myGroup, myGroupScore + commonTopics AS score
ORDER BY score DESC
LIMIT 10
----
