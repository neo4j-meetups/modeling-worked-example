...
----
MATCH (member:Member {name: '<span  value-key="name">A Name</span>'})-[:INTERESTED_IN]->(topic),
...
[source,cypher,subs="attributes,none"]
----
MATCH (member:Member {name: '<span  value-key="name">A Name</span>'})-[:INTERESTED_IN]->(topic), (member)-[:MEMBER_OF]->(group)-[:HAS_TOPIC]->(topic)
WITH member, topic, COUNT(*) AS score
MATCH (topic)<-[:HAS_TOPIC]-(otherGroup)
WHERE NOT (member)-[:MEMBER_OF]->(otherGroup)
RETURN otherGroup.name, COLLECT(topic.name), SUM(score) as score
