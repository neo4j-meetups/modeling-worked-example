...
This is quite a crude scoring mechanism - can we think of something better to go here?
...
WHERE times >= 5 AND NOT (m)-[:INTERESTED_IN]->(topic)
...
* removes any topics that show up less than 5 times or that the member is already interested in
...
With a production sized dataset we'd need to process batches of members at a time.
